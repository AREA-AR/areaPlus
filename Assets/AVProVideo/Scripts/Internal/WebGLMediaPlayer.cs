...
namespace RenderHeads.Media.AVProVideo
{
    ...
    public sealed class WebGLMediaPlayer : BaseMediaPlayer
    {
		...
		public WebGLMediaPlayer(WebGL.ExternalLibrary externalLibrary)
		{
			SetOptions(externalLibrary);
		}

        public void SetOptions(WebGL.ExternalLibrary externalLibrary)
        {
				...
				if (AVPPlayerReady(_playerIndex))
                {
					if (AVPPlayerHasVideo(_playerIndex))
					{
						...
						if (_texture == null)
						{
							...
							{
								Debug.LogWarning("[AVProVideo] WebGL 1.0 not detected and is recommended for best compatibility. Please go to Player Settings > Other Settings > Auto Graphics API and remove WebGL 2.0 from the list.");
							}
