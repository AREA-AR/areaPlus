	...
	public class MediaPlayer : MonoBehaviour
	{
		...
		protected virtual void Update()
		{
			...
			if (m_Control != null)
			{
				...
				UpdateErrors();
				UpdateEvents();
                if (m_Control.IsPlaying() & startPlay == false)
                {
                    Invoke("updateWidthHeight", 1);
                    startPlay = true;
                }                
            }
		}
        ...
        {
            SimpleCloudHandler.width = m_Info.GetVideoWidth();
            SimpleCloudHandler.height = m_Info.GetVideoHeight();
            Debug.Log(m_Info.GetVideoWidth());
        }
...
#region Platform and Path
	...
	// Setup for running builds
		...
		public virtual BaseMediaPlayer CreatePlatformMediaPlayer()
		{
            ...
            mediaPlayer = new WebGLMediaPlayer(_optionsWebGL.externalLibrary);
#elif (UNITY_PS4)
